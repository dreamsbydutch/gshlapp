...
export function useWeeksData (options: WeeksQueryOptions) {
	const season: SeasonInfoDataType = (typeof options.season === "number" || typeof options.season === "undefined") ? getSeason(options.season) : options.season
    ...
    let weeksData: ScheduleWeekType[] = weeks.data.map((obj:{[key: string]: string | number | Date | null}) => formatWeeks(obj)).sort((a:ScheduleWeekType,b:ScheduleWeekType) => a.id - b.id)
    if (options.season) {
